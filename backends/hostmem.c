...
host_memory_backend_memory_complete(UserCreatable *uc, Error **errp)
{
    ...
    if (!verify_policy_hostnodes(policy, sizeof(backend->host_nodes), maxnode,
        errp)) {
        return;
    }

#ifdef HAVE_NUMA_HAS_PREFERRED_MANY
    if (policy == MPOL_PREFERRED && numa_has_preferred_many() > 0) {
        /*
    ...
    if (maxnode &&
        mbind(ptr, sz, policy, backend->host_nodes, maxnode + 1, flags)) {
